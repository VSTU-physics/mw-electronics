...
Рассмотрим электронный поток в замедляющей структуре, точнее взаимодействие волн структуры с волнами пространственного заряда. Чтобы пренебречь взаимодействием с циклотронными волнами поместим систему в достаточно большое продольное магнитное поле.
...
Выпишемм телеграфные уравнения для волны структуры с частотой $$ \omega $$:
...
Решение этих уравнений ищется в виде $$ e^{\gamma z} $$, где в силу слабой связи между пучком и структурой можно представить $$ \gamma = -i\beta_e(1+iC\delta) $$, где $$ C $$ определяет степень связи волн. Произвдные в правой части можно заменить на $$ -i\beta_e $$, получив при этом систему

$$
	\begin{cases}
	\left(\frac{d}{dz} + i\beta\right)a_{1\pm} = \mp c_{12}\frac{d}{dz}(a_{2-} - a_{2+}),\\
	\left(\frac{d}{dz} + i(\beta_e\mp\beta_q)\right)a_{2\pm} = -c_{12}\frac{d}{dz}(a_{1+} + a_{1-}),
	\end{cases}
$$
...
$$
	\begin{cases}
		...
		\frac{d}{dz} a_{1-} = -i\beta a_{1-} + c_{12}a_{2+} - c_{12}a_{2-},\\
		\frac{d}{dz} a_{2+} = -c_{12} a_{1+} - c_{12}a_{1-} - (\beta_e - \beta_q)a_{2+},\\
		\frac{d}{dz} a_{2-} = -c_{12} a_{1+} - c_{12}a_{1-} - (\beta_e + \beta_q)a_{2-}.
	\end{cases}
$$
